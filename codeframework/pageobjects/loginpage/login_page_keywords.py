...
class LoginPageKeywords(BasePage):
    def verify_login_page_elements(self):
        ...
        self.element_attribute_should_be(login_page_location.login_input_password, "placeholder",
                                         login_page_text.login_password_placeholder_text)
        # 验证登录界面中登录按钮的文本
    ...
    def verify_login_failed(self, locker_or_unactiver=True, timeout=config.TIMEOUT):
        ...
        if locker_or_unactiver:
            self.element_text_should_be(login_page_location.login_div_login_error,
                                        login_page_text.login_error_lock_text)
        else:
            self.element_text_should_be(login_page_location.login_div_login_error,
                                        login_page_text.login_error_unactive_text)
